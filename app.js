...
io.on("connection", (socket) => {
  ...
  socket.on("roomJoin", (roomNum) =>{
    roomName = roomNum 
    console.log('룸넘버',roomName);
  ...
  socket.on("setNick", (nick) => {
    ...
    if ( Object.values(nickArray).indexOf(nick) > -1 ) {
      ...
      io.to(roomName).emit("notice", `${nick}님이 입장하였습니다`);

      socket.emit("entrySuccess", nick);
      io.to(roomName).emit("entire", nickArray)
    }
  });
  ...
  socket.on('disconnect', () => {
    ...
    io.to(roomName).emit('notice', `${nickArray[socket.id]}님이 퇴장하셨습니다`);
  ...
  socket.on('send', (data) => {
    ...
    const sendData = { nick: data.myNick, msg: data.msg, roomName: data.roomNum}
    io.to(roomName).emit('newMessage', sendData)
  });
});
